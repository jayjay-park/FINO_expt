...
# ==== Sobolev misfit & regularization toggles ====
USE_H1_DATA_MISFIT      = True    # set True to use H1 residual norm for data misfit
USE_HMINUS1_DATA_MISFIT = False   # set True to use H^{-1} residual norm (overrides H1 if True)
H1_L2_WEIGHT            = 0.1     # weight on L2 part inside H1
H1_GRAD_WEIGHT          = 1.0     # weight on gradient part inside H1
REG_H1_WEIGHT           = 1e-3    # small H1 regularizer on parameter field (set 0.0 to disable)
REG_TV_WEIGHT           = 0.0     # optional TV regularizer on parameter field
USE_ARMIJO_LINESEARCH   = False   # if True, perform two-sided Armijo step instead of optimizer.step()
# ================================================
import math
...
def least_squares_posterior_estimation(model, input_data, true_data, learning_rate,
    ...
    for iteration in range(num_iterations):
        ...
        # loss (pure MLE)
        # Build Sobolev data misfit (supports full or sparse obs)
        if USE_HMINUS1_DATA_MISFIT or USE_H1_DATA_MISFIT:
            # ensure [B,1,H,W] for grid-based residuals
            if full_obs_flag:
                # full field available
                if USE_HMINUS1_DATA_MISFIT:
                    r_img = y_pred - y_obs
                    loss_data = hminus1_loss_from_residual_image(r_img)
                else:
                    loss_data = h1_loss(y_pred, y_obs, include_L2=True, w_L2=H1_L2_WEIGHT, w_grad=H1_GRAD_WEIGHT)
            else:
                # sparse observations at indices (i,j): scatter to residual image then apply Sobolev norm
                # y_pred, y_obs currently reduced to y[:,:,i,j]; we need shapes to build residual field
                # rebuild predictions on the same reduced view to get residuals
                r_img = torch.zeros_like(true_data if true_data.ndim==4 else true_data.unsqueeze(1), device=y_pred.device, dtype=y_pred.dtype)
                # y_pred,y_obs here are [B,1,#pts] after slicing; scatter back
                # Handle batch singleton
                if r_img.ndim == 4:
                    r_img[:, :, i, j] = (y_pred - y_obs)
                else:
                    raise RuntimeError("Unexpected residual image shape for sparse obs.")
                if USE_HMINUS1_DATA_MISFIT:
                    loss_data = hminus1_loss_from_residual_image(r_img)
                else:
                    loss_data = h1_loss(r_img, torch.zeros_like(r_img), include_L2=True, w_L2=H1_L2_WEIGHT, w_grad=H1_GRAD_WEIGHT)
        else:
            loss_data = mse_loss(y_pred, y_obs)

        # small spatial regularizer on the current parameter (in physical space)
        x_phys_for_reg = darcy_mask1_beta(x_free, a_min, a_max, beta=beta)
        reg_term = 0.0
        if REG_H1_WEIGHT > 0.0:
            reg_term = reg_term + REG_H1_WEIGHT * sobolev_H1_norm(x_phys_for_reg).mean()
        if REG_TV_WEIGHT > 0.0:
            reg_term = reg_term + REG_TV_WEIGHT * total_variance(x_phys_for_reg)
        ...
        #         loss_lb = (
        #             hminus1_loss_from_residual_image(y_hat - y_obs) if USE_HMINUS1_DATA_MISFIT else
        #             (h1_loss(y_hat, y_obs, include_L2=True, w_L2=H1_L2_WEIGHT, w_grad=H1_GRAD_WEIGHT) if USE_H1_DATA_MISFIT
        #             else mse_loss(y_hat, y_obs))
        #         ) + (
        #             (REG_H1_WEIGHT * sobolev_H1_norm(a_lb).mean()) if REG_H1_WEIGHT > 0.0 else 0.0
        #         ) + (
        #             (REG_TV_WEIGHT * total_variance(a_lb)) if REG_TV_WEIGHT > 0.0 else 0.0
        #         )
        #         loss_lb.backward()
...
if __name__ == "__main__":
    ...
    noise_std = 0.05 #0.05 0.2 #0.01 #0.3 #0.01
    sub_sampling = True
